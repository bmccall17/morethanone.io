...
- [x] Create demo scenario data at `src/lib/demo/scenarios.ts`. Export three scenario objects, each containing: `name`, `description`, `options` (string array), `ballots` (array of ranking arrays representing mock participants), and `teachableMoment` (string). Scenario 1: "Early Leader, Late Overtake" with 5 options (A-E), 15 ballots where A leads in first preferences but C wins via redistributed secondary preferences from E and B. Scenario 2: "Polarizing Favorite vs Steady Consensus" with 4 options (A-D), 12 ballots where A has most firsts but B wins via consensus. Scenario 3: "Comeback from Third" with 6 options (A-F), 18 ballots where C starts third but wins via layered preferences. Each ballot set verified to produce the described outcome through the `converge()` engine.

- [x] Create a demo engine at `src/lib/demo/engine.ts` that wraps the existing `converge()` function from `src/lib/engine/converge.ts` for step-by-step playback. Export a `DemoRunner` class that takes a scenario, runs `converge()` to get the full result, then exposes: `totalRounds()`, `getRound(n)` (returns round data up to round n), `getExplanation(n)` (returns plain-language explanation of what happened in round n — who was eliminated, why, where votes transferred, how threshold changed). Explanations are generated dynamically from the `ConvergeResult` round data, not hardcoded.

- [x] Create a mock participant view component at `src/components/demo/MockParticipantCard.tsx`. It receives a participant name, their ballot (ranking array), the current round number, and round data. Displays: participant's name, ranked preferences as an ordered list, with strikethrough for eliminated preferences and indigo highlight for the currently active preference. Updates reactively as the round number advances.

- [x] Create the demo composite view page at `src/app/demo/page.tsx`. Includes: (1) scenario selector dropdown, (2) host control panel on the left with round-by-round tallies via a new `DemoTallyView` component (same visual language as `RevealAnimation` but manually stepped), (3) grid of `MockParticipantCard` components on the right showing 8 mock participants, (4) "Previous" and "Next" step buttons with round counter, (5) "Show Explanations" toggle with plain-language narration from `DemoRunner.getExplanation(n)`, (6) "Takeaway" card shown on the final round with the scenario's teachable moment. Also added: `src/components/demo/DemoTallyView.tsx` as a controlled tally bar chart for manual stepping.

- [x] Add a "See how it works" link to the home page at `src/app/page.tsx` below the main CTAs, navigating to `/demo`.
