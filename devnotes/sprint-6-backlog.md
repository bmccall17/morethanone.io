...
- [ ] Add a "Remove" button next to each player in the `PlayerList` component at `src/components/PlayerList.tsx` (visible only to hosts). When clicked, call the remove participant API route. After removal, the player should disappear from the list via the existing realtime subscription. Only show the remove button during `setup` and `ranking` status.
