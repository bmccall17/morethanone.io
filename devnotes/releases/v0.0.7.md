...
### ranking UX
- **mobile drag-and-drop fix:** replaced PointerSensor with TouchSensor + MouseSensor (dnd-kit), added `touchAction: 'none'` on draggable items, added viewport meta to prevent pinch-zoom interference — mobile ranking now works on iOS Safari and Android Chrome
...
### modified files
- `src/app/layout.tsx` - viewport export to prevent pinch-zoom on mobile
...
- `src/components/DraggableRankList.tsx` - max_ranks enforcement, TouchSensor + MouseSensor swap, touchAction: none
